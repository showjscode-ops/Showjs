...
async def home(c:CallbackQuery): await c.answer(); await ensure_user(c.from_user.id,c.from_user.username,c.from_user.full_name); t,_=await dashboard_text(c.from_user.id); await c.message.edit_text(t,parse_mode='HTML',reply_markup=home_kb())
@router.callback_query(F.data=='menu_lainnya')
async def more(c): await c.answer(); await c.message.edit_text('📂 <b>MENU LAINNYA</b>',parse_mode='HTML',reply_markup=other_menu_kb(await is_creator(c.from_user.id)))
