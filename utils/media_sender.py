...
async def deliver_one(bot, chat_id, media, caption=None):
    ...
    try:
        typ = str(media.get("type") or "document").lower()
...
async def deliver_telegram_file_id(bot, chat_id, media, caption=None):
    fid = media.get("telegram_file_id") or media.get("file_id")
    if not fid:
        raise RuntimeError("Telegram file_id tidak tersedia")
    typ = str(media.get("type") or "document").lower()
    if typ == "photo": return await bot.send_photo(chat_id, fid, caption=caption)
    if typ == "video": return await bot.send_video(chat_id, fid, caption=caption)
    if typ == "audio": return await bot.send_audio(chat_id, fid, caption=caption)
